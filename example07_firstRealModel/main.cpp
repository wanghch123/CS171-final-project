...
namespace osc {
  ...
  struct SampleWindow : public GLFCameraWindow
  {
    ...
    {
      if (cameraFrameManip->keyW) {
        cameraFrame.position += -cameraFrame.frame.vz * 0.2f;
        cameraFrame.modified = true;
      }
      if (cameraFrameManip->keyS) {
        cameraFrame.position += cameraFrame.frame.vz * 0.2f;
        cameraFrame.modified = true;
      }
      if (cameraFrameManip->keyA) {
        cameraFrame.position += -cameraFrame.frame.vx * 0.2f;
        cameraFrame.modified = true;
      }
      if (cameraFrameManip->keyD) {
        cameraFrame.position += cameraFrame.frame.vx * 0.2f;
        cameraFrame.modified = true;
      }
